...
async function listAllContracts() {
  const apiResponse = await apiRequest('GET', '/recurring-contract/list', {}, null);
  return apiResponse;
}
...
async function requestPayment(
  ...
  const apiResponse = await apiRequest(
    'POST',
    '/recurring-contract/request-payment',
    {},
    {
      contract_number,
      amount,
...
async function approvePaymentRequest(id) {
  const apiResponse = await apiRequest(
    'GET',
    `/recurring-contract/approve-payment/${id}`,
    {},
    null
  );
  return apiResponse;
